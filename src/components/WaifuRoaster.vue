    ...
    <div class="w-full bg-gray-800 rounded-xl shadow-lg overflow-hidden m-4">
      <div class="p-8">
        ...
        <p class="mt-2 text-white-600">
          Enter your favorite (or least favorite) waifu name and watch it get
          roasted!
        </p>
        <div class="mt-6">
          <input
            v-model="waifuName"
            type="text"
            placeholder="Enter your waifu name"
...
<script>
import { roastWaifu } from "@/services/geminiService";
...
export default {
  ...
  methods: {
    async roastWaifu() {
      if (this.waifuName) {
        this.roast = await roastWaifu(this.waifuName);
      }
    },
  },
};
