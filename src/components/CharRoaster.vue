    ...
    <div class="w-full bg-gray-800 rounded-xl shadow-lg overflow-hidden m-4">
      <div class="p-8">
        ...
        <p class="mt-2 text-white-600">
          Enter your favorite (or least favorite) Characters name and watch it
          get roasted!
        </p>
        <div class="mt-6">
          <input
...
<script>
import { roastChar } from "../services/geminiService";
...
export default {
  ...
  methods: {
    async roastChar() {
      if (this.charName) {
        this.roast = await roastChar(this.charName);
      }
    },
  },
};
